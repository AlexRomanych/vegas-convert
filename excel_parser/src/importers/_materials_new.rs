...
use crate::structures::material_new::Material;
...
pub async fn run(tx: &mut Transaction<'_, Postgres>, path: &str, pool_executor: &PgPool) -> Result<()> {
    ...
    let mut groups: HashMap<String, Material> = HashMap::new();
    let mut categories: HashMap<String, Material> = HashMap::new();
    ...
    {
        let group_code = cell_to_string_by_option(row.get(Material::GROUP_CODE_COL - 1));

        if group_code.eq(Material::STOP_WORD) {
            break;
        }

        let group_code = cell_to_string_by_option(row.get(Material::GROUP_NAME_COL - 1));
        ...
        groups.insert(item_code.clone(), item);
