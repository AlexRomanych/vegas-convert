...
async fn main() -> Result<()> {
    ...
    if !cfg!(debug_assertions) {
        ...
        // __ На dev
        let order_ids_arr = [
            1133, // 449, 450, 467, 445, 456, 462, 438, 461, 452, 475, 442, 444, 446, 447, 443, 451, 454, 459, 470, 476, 436, 455, 463, 440, 472, 439, 448,
                            ...
                            if let Some(material_code) = &item.mc {
                                ...
                                if let Some(material) = materials.get(material_code) {
                                    ...
                                    if let Some(procedure_code) = &item.pc {
                                        ...
                                        let height = if i == 0 {
                                            order_line.get_base_height()
                                        } else {
                                            order_line.get_cover_height()
                                        };
                                        ...
                                        // !!! Debug
                                        // println!("in_scope: {} {:?}", order_line.order_id, in_scope);
                                        // if order_line.line_id == 20814 {
                                            // println!("in_scope: {:?}", in_scope);
                                        // }
                                        // println!("item: {:?}", item);
                                        ...
                                        // !!! Debug
                                        // if order_line.line_id == 20814 && item.pc.clone().unwrap_or_default() == "000000810" {
                                        //     println!("procedure_before: {:#?}", procedure);
                                        // }
                                        ...
                                        // !!! Debug
                                        // if order_line.line_id == 20814 && item.pc.clone().unwrap_or_default() == "000000810" {
                                            // println!("procedure_after: {:#?}", procedure);
                                        // }
                                        ...
                                        if material.is_material() {
                                            ...
                                            let expense_material = ExpenseMaterial {
                                                ...
                                                // __ Отход Без учета Коэффициетна
                                                rest_per_pic:                rest.unwrap_or_default(),
                                                rest:                        rest.unwrap_or_default() * order_line.amount as f64,
                                                // __ Отход С учетом Коэффициетна
                                                // rest_per_pic:                rest.unwrap_or_default()
...
fn _get_procedures_local() -> Vec<Procedure> {
    let text = String::from(
        r#"
                ШиринаПолотна = [ПолотнаСтеганные].{РабочаяШирина};
                [ПолотнаСтеганные].[Ширина] = [ПолотнаСтеганные].{Ширина};
                Припуск =  0.04;
                ...
                Если Длина > 0.5 и  Ширина > 0.5  Тогда


                ПолезныйРасход = ОКР((((Длина)*2 + (Ширина +0.08)+((Ширина-0.9)+0.08) )*(Высота + 0.03))/ [ПолотнаСтеганные].[Ширина],2);
                ОбщийРасход = ОКР(((Длина)*2 + (Ширина+0.08) + ((Ширина-0.9)+0.08) + 0.02)/ЦЕЛ(ШиринаПолотна/(Высота+0.03)),2) + Припуск;
                Отходы = ОбщийРасход - ПолезныйРасход;

                Иначе
                ПолезныйРасход = ОКР(((Длина + Ширина + Припуск)*2*(Высота + 0.03))/ [ПолотнаСтеганные].[Ширина],2);
                ОбщийРасход = ПолезныйРасход*КоэффициентФорматки;
                Отходы = ОбщийРасход - ПолезныйРасход;

                    КонецЕсли;
                [ПолотнаСтеганные] = ПолезныйРасход;
                [ПолотнаСтеганныеОтход] = Отходы;
            "#,
    );
